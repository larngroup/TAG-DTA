...
class dataset_builder():
    """
    Set of Affinity Dataset processing functions
    
    Args:
    - data_path [Dict]: Affinity Dataset Path, Protein Dictionary Path, SMILES Dictionary Path,
                        Affinity Train/Val Clusters Path, Prot BPE Codes Path, Prot BPE Codes Map Path,
                        SMILES BPE Codes Path, SMILES BPE Codes Map Path
    ...
    def get_data(self):
        ...
        if self.data_path['prot_dict'] != '':
            prot_dictionary = json.load(open(self.data_path['prot_dict']))

        if self.data_path['smiles_dict'] != '':
...
def shuffle_split(dataset, train_perc, seed=12345):
    """
    Hold-Out Training/Validation Split - Binding Affinity Dataset
    ...
    """
    dataset = dataset.shuffle(len(dataset), seed=seed, reshuffle_each_iteration=False)
...
#----------- Binding Affinity Dataset TFRecords Save and Load Functions-----------
...
def load_data(FLAGS, path_prot='../data/bind_data/binding_sites_dataset/prot.tfrecords', path_smiles='../data/bind_data/binding_sites_dataset/smiles.tfrecords',
              path_target='../data/bind_data/binding_sites_dataset/target.tfrecords', path_weights='../data/bind_data/binding_sites_dataset/weights.tfrecords'):
    prot = tf.data.TFRecordDataset(path_prot)
    prot = prot.map(parse_prot)
    prot = [add_reg_token(i.numpy(), FLAGS.prot_dict_len) for i in prot][0]
    ...
    data = tf.data.Dataset.from_tensor_slices((prot, smiles, target, weights))
