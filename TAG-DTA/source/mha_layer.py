...
class scaled_dot_product_attention(tf.keras.layers.Layer):
    ...
    def get_config(self):
        config = super(scaled_dot_product_attention, self).get_config()
        config.update({
            'dropout_rate': self.dropout_rate})
        return config


class mhsa_layer(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, d_model, num_heads, dropout_rate, **kwargs):
        super(mhsa_layer, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        self.attention = scaled_dot_product_attention(self.dropout_rate)
    ...
    def get_config(self):
        config = super(mhsa_layer, self).get_config()
        config.update({
