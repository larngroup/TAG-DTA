...
def linear_attn_proj_matrix(dim_k):
    """
...
class linear_attn_head(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, dropout, E_proj, F_proj):
        super(linear_attn_head, self).__init__()
        self.E = E_proj
        self.F = F_proj
    ...
    def get_config(self):
        config = super(linear_attn_head, self).get_config()
        config.update({
            'E_proj': self.E_proj,
            'F_proj': self.F_proj})
        return config
...
class linear_mha_layer(tf.keras.layers.Layer):
    """
    ...
    def __init__(self, d_model, num_heads, dropout_rate, parameter_sharing, dim_k, E_proj, F_proj, **kwargs):
        super(_linear_mha_layer, self).__init__()
    ...
    def build(self, input_shape):
        ...
        if self.parameter_sharing != "layerwise":
            self.E_proj = linear_attn_proj_matrix(self.dim_k)
            self.F_proj = linear_attn_proj_matrix(
        ...
        for _ in range(self.num_heads):
            if self.parameter_sharing == "none":
                self.E_proj = linear_attn_proj_matrix(self.dim_k)
                self.F_proj = linear_attn_proj_matrix(self.dim_k)
            attn = linear_attn_head(self.dropout_rate, self.E_proj, self.F_proj)
    ...
    def get_config(self):
        config = super(linear_mha_layer, self).get_config()
        config.update({
