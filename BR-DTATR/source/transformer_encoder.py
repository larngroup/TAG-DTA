...
class encoder_layer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        if self.full_attention:
            self.mha_layer = mhsa_layer(self.d_model, self.num_heads, self.dropout_rate,
                                                name='enc_self_attn')

        else:
            self.E_proj = linear_attn_proj_matrix(self.dim_k)

            self.mha_layer = linear_mha_layer(self.d_model, self.num_heads, self.dropout_rate, self.parameter_sharing,
                                          self.dim_k,
                                          self.E_proj, self.E_proj, name='enc_self_lattn')

        self.poswiseff_layer = pwffn_block(self.d_model, self.d_ff, self.atv_fun, self.dropout_rate, name='pos_wise_ff')
...
class transformer_encoder(tf.keras.Model):
    ...
    def __init__(self, d_model, num_layers, num_heads, d_ff, atv_fun, dropout_rate,
                 dim_k, parameter_sharing, full_attention, return_intermediate=0, **kwargs):
    ...
    def build(self, input_shape):
        self.enc_layers = [encoder_layer(self.d_model, self.num_heads, self.d_ff, self.atv_fun,
